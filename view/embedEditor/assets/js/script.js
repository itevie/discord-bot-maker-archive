    ...
    buildEmbed = ({ jsonData, only, index = 0 } = {}) => {
        ...
        try {
            ...
            switch (only) {
                ...
                case 'embedThumbnail':
                    ...
                    if (embedObj.thumbnail?.url) {
                        if (embedObj.thumbnail.url.startsWith("{{") && embedObj.thumbnail.url.endsWith("}}")) {
                            embedThumbnailLink.src = "assets/media/variableImage.png";
                        }
                        else embedThumbnailLink.src = embedObj.thumbnail.url;
                ...
                case 'embedImage':
                    ...
                    if (!embedObj.image?.url) hide(embedImageLink.parentElement);
                        ...
                        if (embedObj.image.url.trim().startsWith("{{") && embedObj.image.url.trim().endsWith("}}")) {
                            embedImageLink.src = "assets/media/variableImage.png";
                        }
            ...
            for (const embedObj of jsonObject.embeds) {
                ...
                if (embedObj.thumbnail?.url) {
                    embedThumbnail.src = manageImage(embedObj.thumbnail.url);
                ...
                if (embedObj.image?.url)
                    embedImage.src = manageImage(embedObj.image.url),
...
function manageImage(text) {
    if (text.trim().startsWith("{{") && text.trim().endsWith("}}")) return "assets/media/variableImage.png";
    else return text;
}